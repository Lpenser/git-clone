 ...
 * @param {Function} fn
 */
export default async function  download (repo, dest, opts, fn) {
  ...
  if (clone) {
   ...
   await downloadUrl(url, dest, { extract: true, strip: 1, mode: '666', headers: { accept: 'application/zip' } }).then( async (data) => {
     await  fn()
    }).catch(err => {
      fn(err)
    })
